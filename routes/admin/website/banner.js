 ...
 * @LastEditors: wkiwi
 * @LastEditTime: 2018-12-06 21:58:21
 */
...
router.post('/doadd',tools.multer().single('pic'),async(ctx)=>{
    ...
    var id = null;
    var sql='insert into `banner` (id,title,sort,pic,link,type) value (?,?,?,?,?,?)';
    var result=await DB.query(sql,[id,title,sort,pic,link,type]);
    ctx.redirect(ctx.state.__HOST__+'/admin/website/banner');
})

router.get('/list',async(ctx)=>{
    var page=ctx.query.page||1;
    var pageSize=20;
    var sql1 = 'select count(id) from banner';
    var result1=await DB.query(sql1);//查询总共有多少条
    var count = result1[0]['count(id)'];
    if((page-1)*pageSize<count){//防止用户手动在地址栏输入页码过大
        if(page*pageSize>count){
            var sql='select * from banner limit '+ (page-1)*pageSize+','+(count-(page-1)*pageSize);
        }else {
            var sql='select * from banner limit '+ (page-1)*pageSize+','+pageSize;
        }
        console.log(sql)
...
router.get('/edit',async(ctx)=>{
    ...
    var result=await DB.query(sql);
    await ctx.render('admin/website/editbanner',{
        list: result[0]
    })
})
...
router.post('/doedit',tools.multer().single('pic'),async(ctx)=>{
    ...
    if (pic) {
        var sql='update `banner` set title=?,sort=?,pic=?,link=?,type=? where id='+id;
        var result=await DB.query(sql,[title,sort,pic,link,type]);
    } else {
        var sql='update `banner` set title=?,sort=?,link=?,type=? where id='+id;
        var result=await DB.query(sql,[title,sort,link,type]);
    }
    ctx.redirect(ctx.state.__HOST__+'/admin/website/banner/list');
